...
python3 tools/DrawAnalyzeDiphoton.py Summer16v3.GJets_DR-0p4_HT-600 SR
...
#eosls /store/group/lpcsusyphotons/DiPhoton/TreeMakerRandS_signalskimsv8 > usefulthings/filelistDiphotonSignalSkims.txt
if dosignals: eosdir = '/store/group/lpcsusyphotons/DiPhoton/TreeMakerRandS_signalskimsv8'
else: 
    if doelveto: eosdir = '/store/group/lpcsusyphotons/DiPhoton/Skims_medPhotonElVetoV10'
    else: eosdir = '/store/group/lpcsusyphotons/DiPhoton/Skims_medPhotonNoElVetoV10'
    if fullId: 
    	#eosdir = '/store/group/lpcsusyphotons/DiPhoton/Skims_loosePhotonV10bFullId'
    	eosdir = '/store/group/lpcsusyphotons/DiPhoton/Skims_mediumPhotonV10minmt'
    
'''
eosls /store/group/lpcsusyphotons/DiPhoton/Skims_medPhotonElVetoV10/ > usefulthings/filelistDiphotonSkimsV10.txt
eosls /store/group/lpcsusyphotons/DiPhoton/Skims_medPhotonNoElVetoV10/ > usefulthings/filelistDiphotonSkimsV10NoElVeto.txt
eosls /store/group/lpcsusyphotons/DiPhoton/Skims_loosePhotonV10bFullId > usefulthings/filelistDiphoton_loosePhotonV10bFullId.txt
eosls /store/group/lpcsusyphotons/DiPhoton/Skims_mediumPhotonV10minmt > usefulthings/filelistDiphoton_mediumPhotonV10minmt.txt
'''
...
if dosignals: filefile = open('usefulthings/filelistDiphotonSignalSkims.txt')
    ...
    if fullId: filefile = open('usefulthings/filelistDiphoton_mediumPhotonV10minmt.txt')
...
if 'SR' in SpecialSettings:
    ...
    plotBundle['TwoPho_MinMt'] = ['min(MinMt,199)>>hadchadc(20,0,200)','HardMETPt>130 && NPhotons>=2',False]


print ('fileskey', fileskey)
infilekey = fileskey.split('/')[-1].replace('*','').replace('.root','')
newfilename = 'output_dipho/mediumchunks/weightedHists_'+infilekey+'_'+SpecialSettings+'.root'
